...
PCOMPONENT_COMMON_XDSLMANAGER          g_pComponent_COMMON_xdslmanager  = NULL;
PCCSP_CCD_INTERFACE             pSsdCcdIf                 = (PCCSP_CCD_INTERFACE        )NULL;
PDSLH_LCB_INTERFACE             pDslhLcbIf                = (PDSLH_LCB_INTERFACE        )NULL;
extern char                     g_Subsystem[32];

#define  DATAMODEL_XML_FILE     "/usr/rdk/xdslmanager/RdkXdslManager.xml"
    ...
    )
{
    /* Create component common data model object */
     ...
     g_pComponent_COMMON_xdslmanager = (PCOMPONENT_COMMON_XDSLMANAGER)AnscAllocateMemory(sizeof(COMPONENT_COMMON_XDSLMANAGER));

    if ( ! g_pComponent_COMMON_xdslmanager )
    {
        return ANSC_STATUS_RESOURCES;
    }

    ComponentCommonDmInit( g_pComponent_COMMON_xdslmanager);

     g_pComponent_COMMON_xdslmanager->Name     = AnscCloneString(CCSP_COMPONENT_NAME_XDSLMANAGER);
     g_pComponent_COMMON_xdslmanager->Version  = 1;
     g_pComponent_COMMON_xdslmanager->Author   = AnscCloneString("Your name");
    ...
    )
{
    ...
    char                            CrName[256];
     ...
     g_pComponent_COMMON_xdslmanager->Health = CCSP_COMMON_COMPONENT_HEALTH_Yellow;
    ...
    returnStatus =
        pDslhCpeController->RegisterCcspDataModel
            (
                ...
                CCSP_COMPONENT_NAME_XDSLMANAGER,            /* Component Name    */
                CCSP_COMPONENT_VERSION_XDSLMANAGER,         /* Component Version */
                CCSP_COMPONENT_PATH_XDSLMANAGER,            /* Component Path    */
    ...
    if ( returnStatus == ANSC_STATUS_SUCCESS )
    {
        /* System is fully initialized */
         g_pComponent_COMMON_xdslmanager->Health = CCSP_COMMON_COMPONENT_HEALTH_Green;
    }
    ...
    )
{
    ...
    if(  g_pComponent_COMMON_xdslmanager == NULL)
    {
        return ANSC_STATUS_SUCCESS;
    }
    ...
    if ( g_Subsystem[0] != 0 )
    {
        ...
        snprintf(CpName, sizeof(CpName), "%s%s", g_Subsystem, CCSP_COMPONENT_NAME_XDSLMANAGER);
    }
    else
    {
        snprintf(CrName, sizeof(CrName), "%s", CCSP_DBUS_INTERFACE_CR);
        snprintf(CpName, sizeof(CpName), "%s", CCSP_COMPONENT_NAME_XDSLMANAGER);
    }
    ...
    if (  g_pComponent_COMMON_xdslmanager ) AnscFreeMemory( g_pComponent_COMMON_xdslmanager);

     g_pComponent_COMMON_xdslmanager = NULL;
    pSsdCcdIf                = NULL;
...
{
    return  g_pComponent_COMMON_xdslmanager->Name;
}
...
{
    return  g_pComponent_COMMON_xdslmanager->Version;
}
...
{
    return  g_pComponent_COMMON_xdslmanager->Author;
}
...
{
    return  g_pComponent_COMMON_xdslmanager->Health;
}
...
{
    return  g_pComponent_COMMON_xdslmanager->State;
}
...
{
    return  g_pComponent_COMMON_xdslmanager->LogEnable;
}
...
{
    if( g_pComponent_COMMON_xdslmanager->LogEnable == bEnabled) return ANSC_STATUS_SUCCESS;
     g_pComponent_COMMON_xdslmanager->LogEnable = bEnabled;
    if(bEnabled) g_iTraceLevel = (INT)  g_pComponent_COMMON_xdslmanager->LogLevel;
...
{
    return  g_pComponent_COMMON_xdslmanager->LogLevel;
}
...
{
    if( g_pComponent_COMMON_xdslmanager->LogLevel == LogLevel) return ANSC_STATUS_SUCCESS;
     g_pComponent_COMMON_xdslmanager->LogLevel = LogLevel;
    if( g_pComponent_COMMON_xdslmanager->LogEnable) g_iTraceLevel = (INT)  g_pComponent_COMMON_xdslmanager->LogLevel;
...
{
    return  g_pComponent_COMMON_xdslmanager->MemMinUsage;
}
    ...
    )
{
    ...
    size = AnscGetComponentMemorySize(CCSP_COMPONENT_NAME_XDSLMANAGER);
    if (size == -1 )
        size = 0;
    ...
    )
{
    ...
    /* AnscSetTraceLevel((INT) g_pComponent_COMMON_xdslmanager->LogLevel); */
